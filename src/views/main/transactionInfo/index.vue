      ...
      <div class="topNav">
        <div class="sel">
          ...
          <el-select v-model="selText" placeholder="请选择站点" class="select_element" @change="change">
            <el-option v-for="item in chargeList" :key="item.value" :label="item.label" :value="item.label">
            </el-option>
          </el-select>
        </div>
        <div class="selectDate">
          <div class="block" size="large">
            <el-date-picker v-model="startTime" align="left" type="date" :editable="false" placeholder="" @change="getStartTime(startTime)">
            </el-date-picker>
          </div>
          <span>至</span>
          <div class="block">
            <el-date-picker v-model="endTime" align="left" type="date" :editable="false" placeholder="" @change="getEndTime(endTime)">
            </el-date-picker>
          </div>
        </div>
        <!-- <div class="selTime">
      ...
      <div class="list">
        <ul v-for="item in orderList" :key="item.index" @click="details(item.sn)">
          <li>
            ...
            <div class="liButton" v-if="orderType === 1">
              交易成功
            </div>
            <div class="liButton gre" v-if="orderType === 2">
              使用中
            </div>
            <div class="liButton blu" v-if="orderType === 3">
              预约中
            </div>
          </li>
          <li>
          ...
          <li>
            <span class="">设备编号</span><span>{{item.sn}}</span>
          </li>
          <li v-if="orderType !== 3">
...
<script>
  ...
  export default {
    ...
    methods: {
      data_Init() {
        let chargeList = JSON.parse(STROAGE({
          type: 'getItem',
          key: 'ChargeList'
        }))
        if (chargeList) {
